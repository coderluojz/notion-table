...
interface AppState {
  ...
  updateTableName: (tableId: string, tableName: string) => Promise<boolean>
...
export const useAppStore = create<AppState>((set, get) => ({
  ...
  updateTableName: async (tableId: string, newName: string) => {
    const currentTableList = get().tableList
    const currentTable = currentTableList.find((t) => t.id === tableId)
    if (!currentTable) {
      console.log(`表格${tableId}不存在`)
      return false
    }
    const updatedTable: TableData = { ...currentTable, name: newName }
    try {
  ...
  deleteColumnToActiveTable: async (columnId: string): Promise<boolean> => {
    ...
    const updatedColumns = activeTable.columns.filter(
      (col) => col.id !== columnId
    )
    const updatedColumnOrder = activeTable.columnOrder.filter(
      (id) => id !== columnId
    )
